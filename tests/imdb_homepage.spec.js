...
test('language selection', async ({ page }) => {
  ...
  await page.locator('.ipc-icon.ipc-icon--arrow-drop-down.ipc-btn__icon.ipc-btn__icon--post.navbar__flyout__text-button-post-icon').click();
  await page.locator('ul:nth-child(2) > #language-option-hi-IN > .language-menu-item-span > .ipc-icon').click();
...
//  const top10Link = page.getByRole('link', { name: /Top 10 on IMDb this week/i });
//   await expect(top10Link).toBeVisible();
//   await top10Link.click();
//   await expect(page).not.toHaveURL('https://www.imdb.com/');
// });
